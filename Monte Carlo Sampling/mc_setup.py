...
def compute_log_marginal_scipy(PHI, t, alph, s2):
    ...
    lgp = stats.multivariate_normal.logpdf(t.T, mean=None, cov=C, allow_singular=True)
    #
    return lgp
...
def plot_regression(x_train, t_train, x_test=None, t_test=None, y=None):
    
    lw = 2.5
    if t_test is not None:
        plt.plot(x_test, t_test, '--', color='GoldenRod', lw=lw)
    
    if y is not None:
        plt.plot(x_test, y, '-', color='FireBrick', lw=lw)
    
    plt.plot(x_train, t_train, 'o', ms=8, mew=3, color='black',
